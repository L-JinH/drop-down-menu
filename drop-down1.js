...
/*
for (let z = 0; z < 클릭할 변수.length; z++) {
  클릭할 변수[z].addEventListener("mouseover", function () {
    //마우스를 올리면 할 일.
    적용할 높이를 저장한 변수 = this(이벤트가 일어난 요소).querySelector(".submenu"): 높이를 구할 요소.offsetHeight;
    header.style.height = 높이를 늘릴 요소의 원래 높이. + 적용할 높이를 저장한 변수 + "px"(단위);
  });

  클릭할 변수[z].addEventListener("mouseout", function () {
     //마우스가 나가면 할 일.
    header.style.height = 높이를 늘릴 요소의 원래 높이 + "px"(단위);
  });
}
*/
